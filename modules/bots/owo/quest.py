...
class Quest:
    ...
    def quest_progress(client, message):
        ...
        Quest._spawn(client, Quest.do_quest(client), 'owo-quest-refetch')
    ...
    async def do_quest(client):
        if not client.can_run() or not (client.config['quest'] or client.config['checklist']):
            return
    ...
    def set_quest_flag(client, quest_text):
        if re.search(r"Say 'owo' [0-9]+ times!|[0-9]+ xp from hunting and battling!", quest_text):
            Quest.set_spam_flags(client)
        elif re.search(r"Hunt [0-9]+ [a-zA-Z]+ rank animals!|Manually hunt [0-9]+ times!", quest_text):
            Quest.set_spam_flags(client)
        elif re.search(r"Battle [0-9]+ times!", quest_text):
            Quest.set_spam_flags(client)
