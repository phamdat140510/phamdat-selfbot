...
class Checklist:
    ...
    async def check(client):
        ...
        client.logger.info(f'Daily checklist: spam={daily[0]}, cookie={daily[1]}')
        ...
        if await Checklist._switch_to_weekly(client, message):
            ...
            client.logger.info(f'Weekly checklist: spam={week[0]}, cookie={week[1]}')
            await Checklist._claim(client, message)

        client.checklist_flag = bool(daily[0] or week[0])
        client.checklist_cookie = bool(daily[1])
        client.logger.info(
            f'Checklist: spam={client.checklist_flag}, cookie={client.checklist_cookie}')
        return True

    @staticmethod
    def schedule(client):
        if client.checklist_flag:
        ...
        return int(min(w for w in windows if w > now) - now) + 1