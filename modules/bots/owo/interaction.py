...
class Interaction:
    ...
    def has(self, kind):
        return self._current.get(kind) is not None
    ...
    def ensure(self, kind):
        if kind in self._tasks:
            return
        worker = self._worker_action if kind == 'action' else (
            self._worker_cookie if kind == 'cookie' else self._worker)
        self._tasks[kind] = asyncio.create_task(worker(kind), name=f'owo-interaction-{kind}')
    ...
    async def _worker_action(self, kind):
        try:
            while True:
    ...
    async def _worker_cookie(self, kind):
        try:
            while True:
                senders = [c for c in self._clients if c.can_run() and c.cookie_ready()]
                pending = [c for c in self._clients if self._needs(c, 'cookie')]
                unfetched = [c for c in self._clients if c.quest_enabled() and not c.quest_fetched]
                checklist = any(c.checklist_cookie for c in self._clients)
                if pending or (checklist and not unfetched):
                    if not senders:
                        ...
                        continue
                    for sender in list(senders):
                        if not sender.can_run() or not sender.cookie_ready():
                            continue
                        if not sender.current_channel:
                            continue
                        helpers = [c for c in pending if c is not sender]
                        if not sender.checklist_cookie and not helpers:
                            continue
                        partners = helpers or \
