...
def _click_containing(page, *words):
    for button in (page.eles('css:button') or []):
        try:
            if any(word in (button.text or '').lower() for word in words):
                button.click()
                return True
        except Exception:
            pass
...
def _wait_confirmation(page):
    for _ in range(15):
        ...
        if any(text in body_text for text in ('thanks for voting', 'thank you for voting', 'voted for', 'vote received')):
            return True
        try:
            button = page.ele('css:button.button-primary', timeout=1)
...
def _vote(bot_id, token):
    path = find_browser()
    if not path:
        logger.warning('Browser not found for voting')
        return False
    ...
    click_discord = ('document.querySelectorAll("button").forEach(e=>{'
                     'let t=(e.textContent||"").trim().toLowerCase();'
                     'if(t.indexOf("login with discord")>=0)e.click();});')
    ...
    try:
        ...
        if response.status_code != 200:
            logger.warning(f'Vote: consent failed ({response.status_code})')
            return False
        logger.info('Vote: oauth consent ok')
        ...
        page.get(vote_url)
        page.wait.doc_loaded(timeout=20)
        logger.info('Vote: on vote page')

        _dismiss_consent(page)
        button = _wait_vote_ready(page)
        if button == 'already':
            logger.info('Vote: already voted')
            return False
        if button is None:
            logger.warning('Vote: vote button never became ready within timeout')
            return False

        if not _click_vote(page, button):
            logger.warning('Vote: failed to click vote button')
            return False
        logger.info('Vote: clicked, waiting for confirmation...')
